...
The `TestShell` submodule extends the `WagerrTestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `WagerrTestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `WagerrTestFramework` between
...
The `TestShell` inherits all `WagerrTestFramework` members and methods, such
as:
...
`WagerrTestFramework` object. The full set of argument keywords which can be
...
Unlike the `WagerrTestFramework` class, the `TestShell` keeps the underlying
...
processes and `WagerrTestFramework` convenience methods can be accessed
interactively.
...
underlying `WagerrTestFramework`:
...
`WagerrTestFramework` class. The user can override these in
