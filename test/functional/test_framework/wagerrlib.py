...
def bin2hex(data):
    ...
    assert type(data) is bytes, "wagerrlib.bintohex requires parameter of type bytes"
    l = len(data)
    result = create_string_buffer(2 * l + 1)
    if wagerrlib.Bin2Hex(data, l, result, 2 * l + 1):
        return result.value.decode("utf-8")
    raise Error("wagerrlib bin2hex error")
...
def signTxInput(tx, inputIdx, inputAmount, prevoutScript, key, sigHashType=SIGHASH_FORKID | SIGHASH_ALL):
    ...
    siglen = wagerrlib.SignTx(tx, len(tx), inputIdx, c_longlong(inputAmount), prevoutScript,
                            len(prevoutScript), sigHashType, key, result, 100)
    if siglen == 0:
        raise Error("wagerrlib signtx error")
...
def randombytes(length):
    ...
    worked = wagerrlib.RandomBytes(result, length)
    if worked != length:
        raise Error("wagerrlib randombytes error")
    return result.value
...
def pubkey(key):
    ...
    l = wagerrlib.GetPubKey(key, result, 65)
...
def txid(txbin):
    """Return a transaction id, given a transaction in hex, object or binary form.
       The returned binary txid is not reversed.  Do: hexlify(wagerrlib.txid(txhex)[::-1]).decode("utf-8") to convert to
