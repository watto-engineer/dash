...
# wagerr_hash
RUN git clone https://github.com/wagerr/wagerr_hash
RUN cd wagerr_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} wagerr
RUN useradd -u ${USER_ID} -g wagerr -s /bin/bash -m -d /wagerr wagerr
...
RUN mkdir /wagerr-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /wagerr-src && \
