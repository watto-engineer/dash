...
bool CheckTokens(const CTransactionRef &tx, CValidationState &state, const CCoinsViewCache &view, const int nHeight, CAmount& nWagerrIn, CAmount& nWagerrOut, std::unordered_map<CTokenGroupID, CTokenGroupBalance>& tgMintMeltBalance) {
    nWagerrIn = 0;
    nWagerrOut = 0;
    tgMintMeltBalance.clear();
