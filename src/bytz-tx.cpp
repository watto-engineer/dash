...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s bytz-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  bytz-tx [options] <hex-tx> [commands]  Update hex-encoded bytz transaction\n" +
              "  bytz-tx [options] -create [commands]   Create hex-encoded bytz transaction\n" +
              "\n";
