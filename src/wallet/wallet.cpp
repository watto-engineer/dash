...
{
    // If wallet doesn't have a chain (e.g wagerr-wallet), don't bother to update txn.
    if (HaveChain()) {
...
CAmount CWalletTx::GetAvailableCredit(bool fUseCache, const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++)
    {
        if (!pwallet->IsSpent(hashTx, i) && (allow_used_addresses || !pwallet->IsUsedDestination(hashTx, i) && (GetTokenGroup(tx->vout[i].scriptPubKey) == NoGroup)))
        {
            const CTxOut &txout = tx->vout[i];
            nCredit += pwallet->GetCredit(txout, filter);
            if (!MoneyRange(nCredit))
