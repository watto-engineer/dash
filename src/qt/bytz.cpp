...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Bytz Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Bytz Core - \""), false, OptionsCategory::GUI);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data and blocks directory and parse bytz.conf
