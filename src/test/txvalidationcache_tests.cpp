...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, const CBettingsView& bettingsViewCache, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks);
...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    for (uint32_t test_flags=0; test_flags < (1U << 16); test_flags += 1) {
        ...
        bool ret = CheckInputs(tx, state, &::ChainstateActive().CoinsTip(), bettingsViewCache, true, test_flags, true, add_to_cache, txdata, nullptr);
        ...
        if (ret && add_to_cache) {
            ...
            BOOST_CHECK(CheckInputs(tx, state, &::ChainstateActive().CoinsTip(), bettingsViewCache, true, test_flags, true, add_to_cache, txdata, &scriptchecks));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        CBettingsView bettingsViewCache(bettingsView.get());
        BOOST_CHECK(!CheckInputs(CTransaction(spend_tx), state, &::ChainstateActive().CoinsTip(), bettingsViewCache, true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, nullptr));

        // If we call again asking for scriptchecks (as happens in
        // ConnectBlock), we should add a script check object for this -- we're
        // not caching invalidity (if that changes, delete this test case).
        std::vector<CScriptCheck> scriptchecks;
        BOOST_CHECK(CheckInputs(CTransaction(spend_tx), state, &::ChainstateActive().CoinsTip(), bettingsViewCache, true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, &scriptchecks));
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        CBettingsView bettingsViewCache(bettingsView.get());
        BOOST_CHECK(CheckInputs(CTransaction(invalid_with_cltv_tx), state, &::ChainstateActive().CoinsTip(), bettingsViewCache, true, SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY, true, true, txdata, nullptr));
    }
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        CBettingsView bettingsViewCache(bettingsView.get());
        BOOST_CHECK(CheckInputs(CTransaction(invalid_with_csv_tx), state, &::ChainstateActive().CoinsTip(), bettingsViewCache, true, SCRIPT_VERIFY_CHECKSEQUENCEVERIFY, true, true, txdata, nullptr));
    }
