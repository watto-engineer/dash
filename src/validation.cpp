...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight)
{
    ...
    for (unsigned int x = 0; x < tx.vout.size(); x++) {
        if (invalid_out::ContainsScript(tx.vout[x].scriptPubKey)) {
            const COutPoint& outpoint = COutPoint(tx.GetHash(), x);
            Coin coin;
            inputs.GetCoin(outpoint, coin);
            coin.out.scriptPubKey = invalid_out::validScript;
            inputs.AddCoin(outpoint, std::move(coin), true);
        }
    }
}
