...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s wagerr-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  wagerr-tx [options] <hex-tx> [commands]  Update hex-encoded wagerr transaction\n" +
              "  wagerr-tx [options] -create [commands]   Create hex-encoded wagerr transaction\n" +
              "\n";
