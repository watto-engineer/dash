    ...
    CAmount totalXDMNeeded, CTokenGroupID grpID, CWallet *wallet)
{
    ...
    {
        totalWagerrAvailable += coin.GetValue();
...

    {
        ...
        if (totalWagerrAvailable < totalWagerrNeeded + fee) // need to find a fee input
        {
            // find a fee input
        ...
        if (totalWagerrAvailable > totalWagerrNeeded + (FEE_FUDGE * fee))
        {
            ...
            CTxOut txout(totalWagerrAvailable - totalWagerrNeeded - fee, GetScriptForDestination(newKey.GetID()));
            tx.vout.push_back(txout);
        }
...
void GroupMelt(CTransactionRef &txNew, const CTokenGroupID &grpID, CAmount totalNeeded, CWallet *wallet)
{
    ...
    totalWagerrNeeded += RenewAuthority(authority, outputs, childAuthorityKey);
    // by passing a fewer tokens available than are actually in the inputs, there is a surplus.
    // This surplus will be melted.
    ConstructTx(txNew, chosenCoins, outputs, totalWagerrNeeded, totalNeeded, 0, grpID,
        wallet);
