    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
     ...
     if (request.fHelp || request.params.size() < 5)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n" +
            HelpExampleCli("configuremanagementtoken", "\"MAGIC\" \"MagicToken\" 4 \"https://raw.githubusercontent.com/wagerr/ATP-descriptions/master/WAGERR-testnet-MAGIC.json\" 4f92d91db24bb0b8ca24a2ec86c4b012ccdc4b2e9d659c2079f5cc358413a765 true") +
            "\n"
        );
...
extern UniValue createtokenauthorities(const JSONRPCRequest& request)
{
    ...
    totalWagerrNeeded += RenewAuthority(chosenCoins[0], outputs, renewAuthorityKey);
    ...
    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, totalWagerrNeeded, 0, 0, grpID, pwallet);
...
extern UniValue minttoken(const JSONRPCRequest& request)
{
    ...
    CTokenGroupID grpID;
    CAmount totalTokensNeeded = 0;
    CAmount totalWagerrNeeded = GROUPED_SATOSHI_AMT; // for the mint destination output
    unsigned int curparam = 0;
    std::vector<CRecipient> outputs;
    ...
    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, totalWagerrNeeded, 0, XDMFeeNeeded, grpID, pwallet);
