...
extern UniValue createtokenauthorities(const JSONRPCRequest& request)
{
    ...
    totalBytzNeeded += RenewAuthority(chosenCoins[0], outputs, renewAuthorityKey);
    ...
    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, totalBytzNeeded, 0, 0, grpID, pwallet);
...
extern UniValue minttoken(const JSONRPCRequest& request)
{
    ...
    CTokenGroupID grpID;
    CAmount totalTokensNeeded = 0;
    CAmount totalBytzNeeded = GROUPED_SATOSHI_AMT; // for the mint destination output
    unsigned int curparam = 0;
    std::vector<CRecipient> outputs;
    ...
    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, totalBytzNeeded, 0, XDMFeeNeeded, grpID, pwallet);
