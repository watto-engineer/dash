...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 WAGERR irreversibly.
    ...
    bool fork_active = ::ChainActive().Height() >= Params().GetConsensus().DIP0024Height;
...
static UniValue getgovernanceinfo(const JSONRPCRequest& request)
{
    ...
    bool fork_active = nBlockHeight >= Params().GetConsensus().DIP0024Height;
