...
std::vector<uint8_t> PackAddrData(const T &id, uint8_t type)
{
    ...
    default:
        if (type == GROUP_TYPE) // Groups can be any length
            ...
            throw std::runtime_error("Error packing bytzaddr: invalid address length");
    }
...
// Implements encoding of CTxDestination using bytzaddr.
class BytzAddrEncoder : public boost::static_visitor<std::string>
{
public:
    BytzAddrEncoder(const CChainParams &p) : params(p) {}
...
{
    BytzAddrContent content = DecodeBytzAddrContent(addr, params);
    if (content.hash.size() == 0)
    {
        return CNoDestination{};
    }

    return DecodeBytzAddrDestination(content);
}
...
BytzAddrContent DecodeBytzAddrContent(const std::string &addr, const CChainParams &params)
{
    ...
    auto type = BytzAddrType((version >> 3) & 0x1f);
...
// PackBytzAddrContent allows for testing PackAddrData in unittests due to
