...
Wagerr Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
To profile a running wagerrd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep wagerrd` -- sleep 60
```
